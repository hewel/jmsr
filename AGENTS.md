...
- **Ark UI lazyMount / unmountOnExit**: Never use `lazyMount` or `unmountOnExit` properties on Ark UI components (such as `Dialog`, `Tabs`, `Collapsible`, or `HoverCard`). They have compatibility issues under WebKitGTK (gtkwebkit). Instead, wrap the conditional contents (like `Dialog.Positioner` or `<Portal>`) inside a Solid `<Show when={...}>` (e.g., using `Dialog.Context`'s reactive state `dialog().open`, or custom tab/collapsible open states).
