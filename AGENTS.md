...
- **Ark UI Dialogs & Nesting**: Prefer standard Ark UI Dialog primitives (e.g., `Dialog.Root`, `Dialog.Trigger`, `Dialog.CloseTrigger`) rather than custom ARIA overlays. For nested dialogs, let Ark UI manage the dismissable layer hierarchy natively by keeping both as standard Ark Dialogs under the same JSX tree. Do not write custom `onInteractOutside` handlers to manage nesting. Use controlled `open` state, `lazyMount`, `unmountOnExit`, and local `onKeyDown` Escape handlers on `Dialog.Content` to ensure tests run reliably in JSDOM.
